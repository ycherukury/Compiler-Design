...
class dps_ARM(object):
    ...
    def __release__(self, argument=0):
        ...
        if argument == 0:
            argument = int(random.sample(self.in_use, 1)[0].split("R")[1])

        self.in_use.remove("R"+str(argument))
        self.free.add("R"+str(argument))
        old_one = list(self.mapping.keys())[list(self.mapping.values()).index(argument)]
        
        if "_value" in old_one:
            base_addr_register = old_one.split("_")[0]
            base_addr_register_register_no = self.mapping[base_addr_register]
            self.in_use.remove("R"+str(base_addr_register_register_no))
            self.free.add("R"+str(base_addr_register_register_no))
            self.mapping[base_addr_register] = 0

        store_subroutine(variable_value=old_one, destination_value="R"+str(argument))
        self.mapping[old_one] = 0
...
def dump_tcg_to_file():
    ...
    for value in tcg_code:
        ...
        if dps_temp['op'] != "":
            if dps_temp['op'] == 'LDR' or dps_temp['op'] == 'STR':
                print('{:<6}'.format("@"+str(dps_temp['op'])), end="")
            else:
...
def load_subroutine(variable_value, destination_value, vector=0, vector_address_store=None, offset=0, vector_name=None):
    if vector == 0:
        ...
        temp_node = node_value(op="LDR", destination=destination_value, source_1="["+str(vector_address_store)+str(" ")+str(",")+"#"+str(int(offset - 1)*4)+"]")
        tcg_code.appendright(temp_node)
        tcg_code.last.value.__print__()
        
def store_subroutine(variable_value, destination_value, vector=0, vector_address_store=None, offset=0, vector_name=None):
    if vector == 0:
        # temp_node = node_value(op="STR", destination=destination_value, source_1="=["+str(variable_value)+"]", source_2="#0")
        # tcg_code.appendright(temp_node)
        # tcg_code.last.value.__print__()
        temp_node = node_value(op="STR", destination=destination_value, source_1="["+str(variable_value)+"]")
        tcg_code.appendright(temp_node)
        tcg_code.last.value.__print__()
    
    else:
            
        temp_node = node_value(op="STR", destination=destination_value, source_1="["+str(vector_address_store)+str(" ")+str(",")+"#"+str(int(offset - 1)*4) + "]")
        tcg_code.appendright(temp_node)
        tcg_code.last.value.__print__()
...
def assignment_subroutine(snippet):
    ...
    if mov_or_not == 1:        
        ...
        if "[" in snippet[4] and "]" in snippet[4]:
            vector_assignment = 1
        ...
        if destination_value == 0:
            ...
            if vector_assignment == 1:

                vector_name = snippet[4].split("[")[0]
                vector_index = int(snippet[4].split("[")[1].split("]")[0])
        ...
        if vector_assignment == 1:
            store_subroutine(snippet[4], "R"+str(destination_value), vector=1, vector_address_store="R"+str(vector_base_address), offset=vector_index, vector_name=vector_name)

    print("\n\n")

def relop_subroutine(snippet):
    print("Relop subroutine")
    if snippet[1] == ">":
        op = "RSBS"
    else:
        op = "SUBS"
    ...
    try:
        if "[" in snippet[3] and "]" in snippet[3]:
            ...
            if source_1_temp == 0:
                ...
                destination_value = source_1_temp
    ...
    if destination_temp == 0:
        destination_temp = __dps__.__allocate__(snippet[4])
        load_subroutine(snippet[4], "R"+str(destination_temp))
